...
// image:https://maven-badges.herokuapp.com/maven-central/io.plurima.parallelconsumer/parallel-consumer-parent/badge.svg?style=flat[link=https://mvnrepository.com/artifact/io.plurima.parallelconsumer/parallel-consumer-parent,Latest Parallel Consumer on Maven Central]
...
If you have questions or issues, raise them at https://github.com/parallel-consumer/parallel-consumer/issues[GitHub Issues].
...
This project is available in Maven Central under the `io.plurima.parallelconsumer` group. Snapshot builds are published to Sonatype Central's snapshots repository: https://central.sonatype.com/repository/maven-snapshots/.

Latest released versions can be seen at https://central.sonatype.com/search?q=io.plurima.parallelconsumer[Central Search].

Where `${project.version}` is the version to be used:

* group ID: `io.plurima.parallelconsumer`
...
[source,xml,indent=0]
        <dependency>
            <groupId>io.plurima.parallelconsumer</groupId>
...
[[upgrade_from_confluent_coordinates]]
=== Upgrading from Confluent Coordinates

The community fork is published with new Maven coordinates, but the Java package names are unchanged.
Application code should continue to import classes from `io.confluent.parallelconsumer`.

For Maven users, change only the dependency coordinates:
...
----
implementation "io.plurima.parallelconsumer:parallel-consumer-core:0.5.4.0"
----

Use the same artifact name for the module you already depend on, and replace the group ID:
...
|`io.plurima.parallelconsumer:parallel-consumer-core`

|`io.confluent.parallelconsumer:parallel-consumer-reactor`
|`io.plurima.parallelconsumer:parallel-consumer-reactor`

|`io.confluent.parallelconsumer:parallel-consumer-vertx`
|`io.plurima.parallelconsumer:parallel-consumer-vertx`

|`io.confluent.parallelconsumer:parallel-consumer-mutiny`
|`io.plurima.parallelconsumer:parallel-consumer-mutiny`
|===

No import rewrite is required:
...
If you consumed a snapshot build while testing this fork, remove the Sonatype snapshots repository for release builds and use the released version from Maven Central.
...
* Publish community fork under the `io.plurima.parallelconsumer` Maven group.
* Add Sonatype Central snapshot and release publishing configuration.
* Update module metadata, SCM, issue tracker, and Maven usage documentation for the community fork.
* Add upgrade guidance for moving from `io.confluent.parallelconsumer` Maven coordinates to `io.plurima.parallelconsumer`.
